    ...
    where TEntity : class, IDbEntity
{
    ...
    public virtual async Task<OperationResult<uint>> CreateAsync(TEntity car, CancellationToken ct)
    {
        try
        {
    ...
    public virtual async Task<OperationResult<TOutEntity?>> GetByIdAsync(uint id, CancellationToken ct)
    {
        try
        {
            ...
            return entity == null
                ? OperationResult<TOutEntity?>.NotFound()
                : OperationResult<TOutEntity?>.Success(_mapper.Map<TOutEntity>(entity));
        }
        catch (Exception e)
        {
