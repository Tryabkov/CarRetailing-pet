...
public class AuthController(IAuthService authService, IConfiguration config) : ControllerBase
{
    ...
    {
        var isSuccess = await authService.SignupAsync(signupDto.Password, signupDto.Email, signupDto.Password, ct);
        if (!isSuccess) return BadRequest("User already exists");
